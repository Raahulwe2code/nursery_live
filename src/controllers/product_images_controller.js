...
if(product_id !="" && product_image_id !=""){
  connection.query(' SELECT * FROM product_images WHERE product_image_id = "'+product_image_id+'" AND product_id = "'+product_id+'"', (err, rows, fields) => {
    if (err) {
      console.log(err)
...
if(rows.affectedRows=='1'){
  ...
  res.status(200).send({"response":"product_id or product_image_id not match"})
}
}
})
